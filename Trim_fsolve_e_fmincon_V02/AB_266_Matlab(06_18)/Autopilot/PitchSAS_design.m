...
% q feedback
Cstate = eye(size(Along,1));
C_q=Cstate(3,:);
ss_ue_deg_2_q_deg_s = ss(Acl_alpha,Blong(:,2),C_q,0);
tf_ue_deg_2_q_deg_s = tf(ss_ue_deg_2_q_deg_s)
zpk_ue_deg_2_q_deg_s = zpk(ss_ue_deg_2_q_deg_s)
figure
pzmap(ss_ue_deg_2_q_deg_s)
...
Aa_cl_full=Aa-Ba(:,2)*[k_alpha k_q]*C_fdbck;
damp(Aa_cl_full)

figure
plot(real(eig(Aa)),imag(eig(Aa)),'b*')
hold on
plot(real(eig(Aa_cl_full)),imag(eig(Aa_cl_full)),'rs')
% sgrid
legend('Full dynamics open loop','Full dynamics close loop','Location','best')
