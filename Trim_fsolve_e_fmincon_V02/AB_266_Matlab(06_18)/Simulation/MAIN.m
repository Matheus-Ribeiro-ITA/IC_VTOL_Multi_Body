...
X0 =[X_eq
    U_eq(2:4)
    0];
X = ode4(@nzCAS,T,X0,U_eq,aircraft,X_eq,Y_eq);
Y= zeros (length (T), length(Y_eq)); 
U= zeros (length (T), length(U_eq));
% Vw= zeros (length(T),3);
Ucom = zeros (length (T), length(U_eq));
Upilot = zeros (length (T), length(U_eq));
Ucas = zeros (length (T), length(U_eq));
rc = zeros (length (T), 1);
e = zeros (length (T), 1);

for i_t=1:length(T)
    [~,Yt,Ut,Ucomt,Upilott,Ucast,rct,et] = nzCAS(T(i_t),X(i_t,:).',U_eq ...
        ,aircraft,X_eq,Y_eq);
    Y(i_t,:)= Yt.';
    U(i_t,:)= Ut.';
    Ucom(i_t,:)= Ucomt.';
